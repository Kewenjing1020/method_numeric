...
def analSol(y0,t0,tEnd,w):
	...
	if (x0 > 0):
		...
		# Initial position equal to 0 : value of phi is +/-pi/2 depending on the sign of u0
		if (u0 > 0):
			phi = -pi/2.0
		else:
			phi = pi/2.0

	# Time array where the solution will be defined (must have enough point to capture the oscillation)
